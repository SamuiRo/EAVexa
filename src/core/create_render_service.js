...
import FileJobStore from './job_store.js';
...
/**
 * Wires the default core stack (registry, pool, queue, storage, job_store,
 * notifier) into a RenderService. The one place that constructs these
 * dependencies, shared by the CLI commands and the legacy jobs.json entry point.
 *
 * @param {{ registry?, pool?, queue?, storage?, job_store?, notifier?, queue_options? }} [overrides]
 */
export function create_render_service(overrides = {}) {
  ...
  const job_store = overrides.job_store ?? new FileJobStore();
  const notifier  = overrides.notifier ?? new WebhookNotifier({ job_store });

  return new RenderService({ registry, pool, queue, storage, job_store, notifier });
}
