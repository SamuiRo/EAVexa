...
### Крок 6 — Полірування (1 д) — частково зроблено, за вибором користувача
- [ ] `s3` і `push` драйвери, `/metrics`, rate limit — **свідомо відкладено**:
      локальний деплой з n8n на тій самій машині не потребує ні об'єктного сховища,
      ні rate limit проти зловживань. `output.type: "s3"/"push"` як і раніше
      відхиляються з чіткою `INVALID_REQUEST` (не тихо ігноруються).
- [ ] `templates vendor` (локалізація віддалених шрифтів) — опційно, не робили
- [x] CI: lint → test → реліз (`.github/workflows/ci.yml`; `lint` = `node --check`
      через `npm run lint` — ESLint-конфігу в проєкті ще нема, тож це швидкий
      бездепенденсний гейт, а не повноцінний лінтер стилю). Docker-крок у CI
      не додавали (образ ще жодного разу не збирався в CI — спершу варто
      прогнати `docker compose up --build` вручну, див. Крок 5).
- [x] `2.0.0` + `CHANGELOG.md`

**Разом ≈ 7–9 днів. Придатне до вжитку після Кроку 2, інтегроване з n8n — після Кроку 4.**
Кроки 0-5 + вибіркова частина Кроку 6 (CI, версія, changelog) завершено.
